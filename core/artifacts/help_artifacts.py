...
LOAD_CONTRACT_COMMAND_MSG = 'This command will prompt the user with the options to create a new contract entrie ' \
...
SET_NETWORK_COMMAND_MSG = 'This command will trigger the configuration of the current network environment for the safe-cli.'
# ----------------------------------------------------------------------------------------------------------------------
...
NEW_CONTRACT_COMMAND_MSG = 'This command will trigger the creation of a new contract artifact withing the safe-cli.'
NEW_ACCOUNT_MSG = 'newAccount --private_key=<Account Private Key>'
NEW_ACCOUNT_COMMAND_MSG = 'This command will trigger the creation of a new account artifact withing the safe-cli.'
NEW_PAYLOAD_MSG = 'newPayload'
NEW_PAYLOAD_COMMAND_MSG = 'This command will trigger the creation of a new payload artifact withing the safe-cli.'
NEW_PAYLOAD_TX_MSG = 'newPayloadTx'
NEW_PAYLOAD_TX_COMMAND_MSG = 'This command will trigger the creation of a new payload_tx artifact withing the safe-cli.'
NEW_TOKEN_MSG = 'newToken --address=<Token Address> --abi=<Path to Contract ABI>'
NEW_TOKEN_COMMAND_MSG = 'This command will trigger the creation of a new token artifact withing the safe-cli.'
# ----------------------------------------------------------------------------------------------------------------------
...
DISCLAIMER_MSG = 'This is a proof of concept software, focused on the design of an interface to interact ' \
                 ...
                 'program in a non testing environment such as Mainnet. For now is recommended to only use this' \
...
REMOVE_OWNERS_COMMAND_MSG = 'This command will trigger the transaction method responsable for the deleteion of one' \
                            ' or more --addresss that are provided via user input.'
# ----------------------------------------------------------------------------------------------------------------------
SEND_ETHER_MSG = 'sendEther --address=<Address To> --private_key=<Private Key from> --ether=(supported from --wei to --tether)'
SEND_ETHER_COMMAND_MSG = 'This command will trigger the trasaction method to make a send for ether procedure between two parties'
...
class InformationArtifacts:
    ...
    def command_view_disclaimer(self):
        self.logger.info(DISCLAIMER_MSG)
    ...
    def command_view_general_information(self):
        ...
        self.logger.info('|\t {0}{1}|'.format(LOAD_CONTRACT_COMMAND_MSG, ' ' * (140 - len(LOAD_CONTRACT_COMMAND_MSG) - 1)))

        # INFO: loadSafe
        information_data = ' (#) {0}'.format(LOAD_SAFE_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(LOAD_SAFE_COMMAND_MSG, ' ' * (140 - len(LOAD_SAFE_COMMAND_MSG) - 1)))

        # INFO: setNetwork
        information_data = ' (#) {0}'.format(SET_NETWORK_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(SET_NETWORK_COMMAND_MSG, ' ' * (140 - len(SET_NETWORK_COMMAND_MSG) - 1)))

        # INFO: viewNetworks
        information_data = ' (#) {0}'.format(VIEW_NETWORKS_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(VIEW_NETWORKS_COMMAND_MSG, ' ' * (140 - len(VIEW_NETWORKS_COMMAND_MSG) - 1)))

        # INFO: viewAccounts
        information_data = ' (#) {0}'.format(VIEW_ACCOUNTS_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info(
            '|\t {0}{1}|'.format(VIEW_ACCOUNTS_COMMAND_MSG, ' ' * (140 - len(VIEW_ACCOUNTS_COMMAND_MSG) - 1)))

        # INFO: viewContracts
        information_data = ' (#) {0}'.format(VIEW_CONTRACT_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(VIEW_CONTRACT_COMMAND_MSG, ' ' * (140 - len(VIEW_CONTRACT_COMMAND_MSG) - 1)))

        # INFO: viewTokens
        information_data = ' (#) {0}'.format(VIEW_TOKENS_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(VIEW_TOKENS_COMMAND_MSG, ' ' * (140 - len(VIEW_TOKENS_COMMAND_MSG) - 1)))

        # INFO: newContract
        information_data = ' (#) {0}'.format(NEW_CONTRACT_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(NEW_CONTRACT_COMMAND_MSG, ' ' * (140 - len(NEW_CONTRACT_COMMAND_MSG) - 1)))

        # INFO: newAccount
        information_data = ' (#) {0}'.format(NEW_ACCOUNT_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(NEW_ACCOUNT_COMMAND_MSG, ' ' * (140 - len(NEW_ACCOUNT_COMMAND_MSG) - 1)))

        # INFO: newPayload
        information_data = ' (#) {0}'.format(NEW_PAYLOAD_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(NEW_PAYLOAD_COMMAND_MSG, ' ' * (140 - len(NEW_PAYLOAD_COMMAND_MSG) - 1)))

        # INFO: newPayloadTx
        information_data = ' (#) {0}'.format(NEW_PAYLOAD_TX_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(NEW_PAYLOAD_TX_COMMAND_MSG, ' ' * (140 - len(NEW_PAYLOAD_TX_COMMAND_MSG) - 1)))

        # INFO: newToken
        information_data = ' (#) {0}'.format(NEW_TOKEN_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(NEW_TOKEN_COMMAND_MSG, ' ' * (140 - len(NEW_TOKEN_COMMAND_MSG) - 1)))

        # INFO: exit
        information_data = ' (#) {0}'.format(QUIT_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(QUIT_COMMAND_MSG, ' ' * (140 - len(QUIT_COMMAND_MSG) - 1)))
    ...
    def command_view_safe_information(self):
        ...
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(LOAD_OWNER_COMMAND_MSG, ' ' * (140 - len(LOAD_OWNER_COMMAND_MSG) - 1)))

        # INFO: unloadOwner
        information_data = ' (#) {0}'.format(UNLOAD_OWNER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(UNLOAD_OWNER_COMMAND_MSG, ' ' * (140 - len(UNLOAD_OWNER_COMMAND_MSG) - 1)))

        # INFO: nonce
        information_data = ' (#) {0}'.format(NONCE_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(NONCE_COMMAND_MSG, ' ' * (140 - len(NONCE_COMMAND_MSG) - 1)))

        # INFO: code
        information_data = ' (#) {0}'.format(CODE_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(CODE_COMMAND_MSG, ' ' * (140 - len(CODE_COMMAND_MSG) - 1)))

        # INFO: version
        information_data = ' (#) {0}'.format(VERSION_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(VERSION_COMMAND_MSG, ' ' * (140 - len(VERSION_COMMAND_MSG) - 1)))
        ...
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(VIEW_SENDER_COMMAND_MSG, ' ' * (140 - len(VIEW_SENDER_COMMAND_MSG) - 1)))

        # INFO: updateSafe
        information_data = ' (#) {0}'.format(UPDATE_SAFE_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(UPDATE_SAFE_COMMAND_MSG, ' ' * (140 - len(UPDATE_SAFE_COMMAND_MSG) - 1)))
        ...
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(IS_OWNER_COMMAND_MSG, ' ' * (140 - len(IS_OWNER_COMMAND_MSG) - 1)))

        # INFO: areOwners
        information_data = ' (#) {0}'.format(ARE_OWNER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(ARE_OWNERS_COMMAND_MSG, ' ' * (140 - len(ARE_OWNERS_COMMAND_MSG) - 1)))

        # INFO: getOwners
        information_data = ' (#) {0}'.format(GET_OWNER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(GET_OWNER_COMMAND_MSG, ' ' * (140 - len(GET_OWNER_COMMAND_MSG) - 1)))

        # INFO: getThreshold
        information_data = ' (#) {0}'.format(GET_THRESHOLD_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(GET_THRESHOLD_COMMAND_MSG, ' ' * (140 - len(GET_THRESHOLD_COMMAND_MSG) - 1)))

        # INFO: addOwner
        information_data = ' (#) {0}'.format(ADD_OWNER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(ADD_OWNER_COMMAND_MSG, ' ' * (140 - len(ADD_OWNER_COMMAND_MSG) - 1)))

        # INFO: addOwners
        information_data = ' (#) {0}'.format(ADD_OWNERS_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(ADD_OWNERS_COMMAND_MSG, ' ' * (140 - len(ADD_OWNERS_COMMAND_MSG) - 1)))

        # INFO: addOwnerWithThreshold
        information_data = ' (#) {0}'.format(ADD_OWNER_WITH_THRESHOLD_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(ADD_OWNER_WITH_THRESHOLD_COMMAND_MSG, ' ' * (140 - len(ADD_OWNER_WITH_THRESHOLD_COMMAND_MSG) - 1)))

        # INFO: changeOwner/swapOwner
        information_data = ' (#) {0}'.format(SWAP_OR_CHANGE_OWNER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(SWAP_OR_CHANGE_OWNER_COMMAND_MSG, ' ' * (140 - len(SWAP_OR_CHANGE_OWNER_COMMAND_MSG) - 1)))

        # INFO: removeOwner
        information_data = ' (#) {0}'.format(REMOVE_OWNER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(REMOVE_OWNER_COMMAND_MSG, ' ' * (140 - len(REMOVE_OWNER_COMMAND_MSG) - 1)))

        # INFO: removeOwners
        information_data = ' (#) {0}'.format(REMOVE_OWNERS_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(REMOVE_OWNERS_COMMAND_MSG, ' ' * (140 - len(REMOVE_OWNERS_COMMAND_MSG) - 1)))

        # INFO: sendEther
        information_data = ' (#) {0}'.format(SEND_ETHER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(SEND_ETHER_COMMAND_MSG, ' ' * (140 - len(SEND_ETHER_COMMAND_MSG) - 1)))

        # INFO: depositEther
        information_data = ' (#) {0}'.format(DEPOSIT_ETHER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(DEPOSIT_ETHER_COMMAND_MSG, ' ' * (140 - len(DEPOSIT_ETHER_COMMAND_MSG) - 1)))

        # INFO: withdrawEther
        information_data = ' (#) {0}'.format(WITHDRAW_ETHER_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(WITHDRAW_ETHER_COMMAND_MSG, ' ' * (140 - len(WITHDRAW_ETHER_COMMAND_MSG) - 1)))

        # INFO: sendToken
        information_data = ' (#) {0}'.format(SEND_TOKEN_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(SEND_TOKEN_COMMAND_MSG, ' ' * (140 - len(SEND_TOKEN_COMMAND_MSG) - 1)))

        # INFO: depositToken
        information_data = ' (#) {0}'.format(DEPOSIT_TOKEN_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(DEPOSIT_TOKEN_COMMAND_MSG, ' ' * (140 - len(DEPOSIT_TOKEN_COMMAND_MSG) - 1)))

        # INFO: withdrawToken
        information_data = ' (#) {0}'.format(WITHDRAW_TOKEN_MSG)
        self.logger.info('| {0}{1}|'.format(information_data, ' ' * (140 - len(information_data) - 1)))
        self.logger.info('|\t {0}{1}|'.format(WITHDRAW_TOKEN_COMMAND_MSG, ' ' * (140 - len(WITHDRAW_TOKEN_COMMAND_MSG) - 1)))
